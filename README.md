...
**项目已于 2026-09-15 按用户要求停止。** 定时复核已暂停，行情采集、模拟交易和本地网页服务已关闭。下文运行说明仅作为历史文档；未经用户明确恢复，不再启动。代码、公开数据与虚拟账本均保留。参见 [完成度评估与停机记录](docs/project_assessment_2026-09-15.md)。

新增 [因子与策略研究](docs/factor_lab.md)：每 10 分钟比较因子增减和策略对照，已加入 19 个固定方案；仅用已记录信号与后续完整行情计结果，冻结候选后再做前向验证。网页分为“因子与策略”和“趋势跟踪”两个工作区。

新增 [本地策略观测站](docs/dashboard.md)：在 `quant` 环境运行 `python -I scripts/dashboard.py`，打开 **http://127.0.0.1:8765**。交互展示独立杠杆场景、净值曲线、候选指标、风险和复核日志，每 5 秒刷新。只读取公开报告与模拟账本，不加载账户凭据。

新增 [短线复核与杠杆模拟](docs/tactical_lab.md)：动态筛选高波动山寨币，分析 5m/15m 量价、订单流和持仓变化，使用明确复核后的一次性判断比较 1×/2×/3× 独立模拟。用户已授权当前任务每 10 分钟复核及新模拟开仓；此 Codex 计划不随仓库分发。入口 `python -I scripts/tactical_lab.py scan`。没有真实下单路径，不会自动把评分转成账户交易。
